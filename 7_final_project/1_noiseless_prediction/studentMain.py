...
def estimate_next_pos(measurement, OTHER):
    ...
    # OTHER = [[past_xy], heading, turning, distance]
    OTHER = [[], heading, turning, distance]
    past_xy = OTHER[0]
    past_xy.append(measurement)
    for i in range(len(past_xy) - 1):
        distance = distance_between(past_xy[i], past_xy[i+1])
